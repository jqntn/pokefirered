...
COMMON_DATA bool8 gDisableMusic = FALSE;

static void
Task_Fanfare(u8 taskId);
static void
CreateFanfareTask(void);
static void
Task_DuckBGMForPokemonCry(u8 taskId);
static void
...
u16
GetCurrentMapMusic(void)
{
  return sCurrentMapMusic;
}

void
PlayNewMapMusic(u16 songNum)
{
...
void
FadeOutAndPlayNewMapMusic(u16 songNum, u8 speed)
{
...
void
FadeOutAndFadeInNewMapMusic(u16 songNum, u8 fadeOutSpeed, u8 fadeInSpeed)
{
...
{
  if (sMapMusicState == 6 || sMapMusicState == 5 || sMapMusicState == 7) {
    return FALSE;
  }
...
void
PlayFanfareByFanfareNum(u8 fanfareNum)
{
...
void
StopFanfareByFanfareNum(u8 fanfareNum)
{
  m4aSongNumStop(sFanfares[fanfareNum].songNum);
}

void
PlayFanfare(u16 songNum)
{
  s32 i;

  for (i = 0; (u32)i < ARRAY_COUNT(sFanfares); i++) {
    if (sFanfares[i].songNum == songNum) {
      PlayFanfareByFanfareNum((u8)i);
      CreateFanfareTask();
      return;
    }
  }
...
bool8
IsFanfareTaskInactive(void)
{
  return FuncIsActiveTask(Task_Fanfare) != TRUE;
}
...
void
FadeInNewBGM(u16 songNum, u8 speed)
{
  if (gDisableMusic) {
    songNum = 0;
  }
...
void
FadeOutBGMTemporarily(u8 speed)
{
  m4aMPlayFadeOutTemporarily(&gMPlayInfo_BGM, speed);
}

bool8
IsBGMPausedOrStopped(void)
{
  if (gMPlayInfo_BGM.status & MUSICPLAYER_STATUS_PAUSE) {
    return TRUE;
  }

  return (gMPlayInfo_BGM.status & MUSICPLAYER_STATUS_TRACK) == 0;
}
...
PlayCry_Normal(u16 species, s8 pan)
{
  ...
  PlayCryInternal(
    species, pan, CRY_VOLUME, CRY_PRIORITY_NORMAL, CRY_MODE_NORMAL);
  gPokemonCryBGMDuckingCounter = 2;
  RestoreBGMVolumeAfterPokemonCry();
}

void
PlayCry_NormalNoDucking(u16 species, s8 pan, s8 volume, u8 priority)
{
...
void
PlayCry_ReleaseDouble(u16 species, s8 pan, u8 mode)
{
  if (mode == CRY_MODE_DOUBLES) {
    PlayCryInternal(species, pan, CRY_VOLUME, CRY_PRIORITY_NORMAL, mode);
  } else {
...
void
PlayCry_Script(u16 species, u8 mode)
{
...
void
PlayCryInternal(u16 species, s8 pan, s8 volume, u8 priority, u8 mode)
{
  ...
  SetPokemonCryVolume((u8)volume);
  SetPokemonCryPanpot(pan);
  SetPokemonCryPitch((s16)pitch);
  SetPokemonCryLength((u16)length);
  SetPokemonCryProgress(0);
  SetPokemonCryRelease((u8)release);
  SetPokemonCryChorus((s8)chorus);
...
bool8
IsCryPlayingOrClearCrySongs(void)
{
...
void
PlaySE12WithPanning(u16 songNum, s8 pan)
{
  m4aSongNumStart(songNum);
...
void
PlaySE1WithPanning(u16 songNum, s8 pan)
{
  m4aSongNumStart(songNum);
  m4aMPlayImmInit(&gMPlayInfo_SE1);
  m4aMPlayPanpotControl(&gMPlayInfo_SE1, TRACKS_ALL, pan);
...
void
PlaySE2WithPanning(u16 songNum, s8 pan)
{
  m4aSongNumStart(songNum);
  m4aMPlayImmInit(&gMPlayInfo_SE2);
  m4aMPlayPanpotControl(&gMPlayInfo_SE2, TRACKS_ALL, pan);
...
bool8
IsSpecialSEPlaying(void)
{
  if (gMPlayInfo_SE3.status & MUSICPLAYER_STATUS_PAUSE) {
    return FALSE;
  }

  return (gMPlayInfo_SE3.status & MUSICPLAYER_STATUS_TRACK) != 0;
}

void
SetBGMVolume_SuppressHelpSystemReduction(u16 volume)
{
...
void
BGMVolumeMax_EnableHelpSystemReduction(void)
{
